...
const string strMessageMagic = "NUD Signed Message:\n";
...
{
    // NUD: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NUD: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NUD: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NUD: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6df6d372a1430a3a59c5d82f88cf4a0f01ba96c27aec679fe09a1f679743adda");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // NUD: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NUDMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NUDMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NUDMiner(CWallet *pwallet)
{
    printf("NUDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nud-miner");
    ...
    try { loop {
        ...
        printf("Running NUDMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NUDMiner, pwallet));
}
